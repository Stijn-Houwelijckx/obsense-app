...
const GenreCollections = ({ navigation, route }) => {
  ...
  const [genre, setGenre] = useState(null); // State to store genre data
  ...
  const loadCollections = useCallback(async () => {
    ...
    const result = await getCollectionsByGenre(genreId, page, 8); // Get collection data from API (page 1, limit 20)

    if (result.status === "success") {
      setGenre(result.data.genre); // Set genre data
